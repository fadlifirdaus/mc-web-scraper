...
df = pd.read_csv(f"update_harga_chandra.csv")
...
def updatePrice(kode_internal,kode_biller, biller, harga_jual, harga_enduser, harga_f1, harga_f2, harga_f3):
    ...
    select.select_by_value('telepay_merchant_product.product_code')
    ...
    for i in row:
        ...
        if x[5].text == kode_biller and x[4].text == kode_internal and x[3].text == biller:
            i.find_element(By.XPATH,"./td/div/button[@title='Ubah Produk H2H']").click()
            time.sleep(1)
...
while True:
    if not driver.current_url == "http://117.102.115.218:8031/web-telechandra-v3/login.html":
        driver.get("http://117.102.115.218:8031/web-telechandra-v3/teleshop/master/produkh2h_url/core/fb925yXGJMNH15U8efJYWEqOLT2B6JXGTUHLtgztgh2mnLQw") 
        time.sleep(2)
